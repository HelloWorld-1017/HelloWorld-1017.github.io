...
Receiver operating characteristic curve(AKA ROC curve，受试者操作特征曲线)用于表征二元分类系统的诊断能力。ROC曲线上的数据点的横坐标是false positive rate (FPR)，纵坐标是true positive rate (TPR)，每个数据点都对应着一个都对应着一个threshold。TPR也被称作也被称作sensitivity，recall或者probability of detection；FPR也被称作probability of false alarm，fall-out或者1-specificity。

ROC也可以被认为是决策规则的I型误差函数的幂函数图(当性能仅从总体样本中计算时，它可以被认为是这些量的估计量)。【*The ROC can also be thought of as a plot of the power as a function of the Type I Error of the decision rule (when the performance is calculated from just a sample of the population, it can be thought of as estimators of these quantities).*】通常而言，如果detection和false alarm的概率分布已知，那么可以通过在y轴上绘制detection probability的累积分布函数以及在x轴上绘制false-alarm的累积分布函数生成ROC曲线。

ROC分析为挑选可能的最优(possibly optimal)模型、独立于成本背景或类别分布丢弃次优模型提供了有利的工具。ROC分析以一种直接和自然的方式与诊断决策的成本/效益分析相关。【ROC analysis is related in a direct and natural way to cost/benefit analysis of diagnostic [decision making](https://en.wikipedia.org/wiki/Decision_making).】

ROC曲线还被称为是relative operating characteristic curve，因为这条曲线对比了两种operating characteristics——TPR和FPR——随criterion变化的情况（[Receiver operating characteristic - Wikipedia](https://en.wikipedia.org/wiki/Receiver_operating_characteristic)）。

在机器学习领域，ROC曲线也有着广泛的应用。在训练好一个分类模型后，我们通常会使用指定的测试集数据来测试算法的性能。一个常用的方法是计算总体性能(gross measure of performance)，比如二次损失(quadratic loss)或者准确率，这些指标测量的是算法在整个训练集上的平均性能。我们可以通过绘制ROC曲线和计算性能指标进一步地分析分类器的性能。比如，我们可以找到最大化分类准确率的threshold，或者评估分类器在high sensitivity和high specificity区域的表现（[ROC Curve and Performance Metrics: Introduction to ROC Curve - MathWorks](https://ww2.mathworks.cn/help/stats/performance-curves.html#mw_0b428fd7-9376-4f08-9f70-4f4bcfea7547)）。

## Area Under ROC Curve (AUC)

ROC曲线与FPR轴所围成的面积对应着ROC关于FPR在区间$[0,1]$上的积分值。AUC值提供了综合所有可能threshold下分类器的performance measure。AUC的取值范围是$[0,1]$，AUC值越大，classifier的性能越好 （[ROC Curve and Performance Metrics: Area Under ROC Curve (AUC) - MathWorks](https://ww2.mathworks.cn/help/stats/performance-curves.html#mw_c996e1a2-3800-4e33-bd73-68cc67bb1ccc)）。

- 完美的分类器总是能给正类observation打上正类的标签，并且对于任何阈值都具有1的TPR。因此，完美的classifier所对应的ROC曲线穿过平面上的点$[0,0]$，$[0,1]$和$[1,1]$，AUC值为1；
- 随机分类器（random classifier）能够返回一个random score，并且对于任何的threshold都具有相同的FPR和TPR值。因此，随机分类器所对应的ROC曲线就落在diagonal line上，AUC值为0.5。
...
ROC曲线的绘制依赖于FPR和TPR值的计算，而FPR和TPR的计算则依赖于混淆矩阵。二分类问题的混淆矩阵如下表所示：

![image-20230105104620218](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230105104620218.png)
...
![image-20230105103025989](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230105103025989.png)

> 默认情况下，混淆矩阵中行和列的顺序都是“先正类后负类”，并且TPR和FPR都是针对“正类”进行计算的。如果我们要针对此时的“负类”计算TPR和FPR，则需要首先将混淆矩阵行和列的顺序对换，再进行相应的计算：
>
> ![image-20230105102959896](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230105102959896.png)
>
> 这时候需要注意的是，第一行计算的是FPR，第二行计算的是TPR，和上面的情况是相反的。

除了FPR和TPR，基于混淆矩阵还可以计算出许多其他的指标：

注：这些指标也都是在默认的“正类”和“负类”下进行计算的。
{: .notice--warning}

![image-20230104222716589](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230104222716589.png)

## ROC curve

正如前文所述，ROC曲线上的每个数据点都是一个FPR-TPR pair，并且对应着一个threshold。要理解这一点，我们需要从分类器的预测机制说起。

通常而言，如果我们将新值（即测试集数据）输入到训练好的分类器中，后者将会给出预测标签。但是更精确地讲，在给出预测标签之前，分类器首先会对“测试数据属于某一类别”这个事件给出一个socore，然后根据score和预设的threshold来预测测试数据的分类。因此可以预想的是，threshold的设置会影响到最终的分类结果。例如，如果二元分类器给observation的score是0.2，那么当threshold为0时（typically），分类器会将其预测为P类；而若threshold为0.25，分类器则会将其预测为N类（[ROC Curve and Performance Metrics: Receiver Operating Characteriestic (ROC) Curve - MathWorks](https://ww2.mathworks.cn/help/stats/performance-curves.html#mw_7d6e3119-05c2-451f-956e-5b2f34edb8a7)）。

上述这种根据score和threshold进行预测的方式也被称作是软分类。当我们不断调整threshold，测试集的预测标签会发生改变，混淆矩阵也会发生相应的改变，进而导致FPR和TPR值的变化，因此我们就可以获得ROC曲线上的一个一个不同的FPR-TPR pair。
...
本节以二元分类问题为例解释ROC曲线，示例参考：[rocmetrics: Plot ROC Curve for Binary Classifier - MathWorks](https://ww2.mathworks.cn/help/stats/rocmetrics.html#mw_42218da9-f74c-4ad9-8009-5e7e7e1aebdd)。

## Code Implementation

二分类数据集`ionosphere.mat`：
...
之后划分数据集，使用训练集训练一个binary SVM classifier，并使用`predict`函数预测测试集数据的标签`Predictions`和`Scores`：

[predict - MathWorks](https://ww2.mathworks.cn/help/stats/classreg.learning.classif.compactclassificationsvm.predict.html): Classify observations using support vector machine (SVM) classifier.
{: .notice--primary}
...
![image-20230105190707686](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230105190707686.png)
...
MATLAB函数`rocmetrics`用于计算二分类和多分类classifier的ROC曲线和performance metrics：[rocmetrics - MathWorks](https://ww2.mathworks.cn/help/stats/rocmetrics.html)：
...
求解ROC曲线下的面积AUC对应着一个数值积分问题，MATLAB函数`trapz`（[trapz - MathWorks](https://ww2.mathworks.cn/help/matlab/ref/trapz.html)）可以满足这个需要。但需要注意的是，该函数计算出的积分值符号是和积分的方向是有关的：
...
![image-20230105202134903](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230105202134903.png)
...
![image-20230105211029251](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230105211029251.png)
...
![image-20230105220255127](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230105220255127.png)
...
![image-20230106100654870](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230106100654870.png)

并在自定义过程中使用`AdjustedScore`：

![image-20230106100741898](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230106100741898.png)

`rocmetrics`函数的输入仍采用未经调整的`Scores`：

![image-20230106100833786](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230106100833786.png)

可以看到，ROC曲线的绘制结果以及AUC值并没有发生变化：

![image-20230105224833939](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230105224833939.png)

## Code Implementation

> 在本部分，我们使用多分类数据集`fisheriris.mat`来验证，使用`rocmetrics`函数实现的代码来自：[rocmetrics: Plot ROC Curves for Multiclass Classifier - MathWorks](https://ww2.mathworks.cn/help/stats/rocmetrics.html#mw_5e951e43-2be4-45aa-973a-85d45b9d14f2)。

尽管这里采用了交叉验证的方法，和上面的二分类问题有所不同，但第一步还是求得`Scores`：
...
> [ROC Curve and Performance Metrics: Thresholds, Fixed Metric, and Fixed Metric Values - MathWorks](https://ww2.mathworks.cn/help/stats/performance-curves.html#mw_4ed5490d-fa83-4f34-9fad-cc3c63daa2d2)

在默认情况下，`rocmetrics`函数会依次将adjusted score matrix中每一个distinct value作为threshold：

![image-20230106094408354](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230106094408354.png)

但除此之外，还需要两个additional Thresholds，使得ROC曲线可以通过TPR-FRP曲面上的点$[0,0]$和$[1,1]$：

![image-20230106094608419](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230106094608419.png)
...
![image-20230106095839299](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230106095839299.png)
...
在上面的过程中，无论是简单的二分类问题还是多分类问题，都是只绘制了单个分类器的ROC曲线。除此之外，我们还可以绘制不同分类器的ROC曲线，以对比不同classifiers的分类性能。MATLAB官方示例[rocmetrics: Compare Classification Models](https://ww2.mathworks.cn/help/stats/rocmetrics.html#mw_d693d08c-37ce-44ae-922f-31756b6a246f)就实现了多个分类器分类性能的对比：
...
![image-20230106103831816](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230106103831816.png)
...
上图中对比了三个分类器关于分类`<=50K `的ROC曲线，结论是分类器Generalized Additive Model具有最高的AUC值，因而具有最好的average performance；而Decision Tree Model的效果是最差的。并且，根据前面的结论，在二分类问题的情况下，同一个分类器的两个ROC曲线具有对称性，且AUC值相同，因此只考察分类`<=50K `的ROC曲线即可。但是，我们仍然可以绘制出`>50K `分类的曲线：

![image-20230106110727332](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230106110727332.png)
...
在多分类问题的背景下比较不同分类器的平均性能可能更加具有实践意义。多分类分类器的平均性能需要综合考虑各个分类的ROC曲线，平均的方式有三种 ([average: Averaging method - MathWorks](https://ww2.mathworks.cn/help/stats/rocmetrics.average.html#mw_9320ccb8-e4af-41eb-a4fc-87395190101b))：

![image-20230106114625023](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230106114625023.png)

[ROC Curve and Performance Metrics: Average of Performance Metrics - MathWorks](https://ww2.mathworks.cn/help/stats/performance-curves.html#mw_0b698efc-b4aa-476f-ac10-7c1c3e5d3778)中有更详细的解释：

![image-20230106101550286](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230106101550286.png)
...
- 第二种方式是通过`rocmetrics`函数得到 one-versus-all ROC curve后，使用`average`函数计算average ROC curve（[average - MathWorks](https://ww2.mathworks.cn/help/stats/rocmetrics.average.html)）：
...
![image-20230106120435444](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/DeLLLaptop/image-20230106120435444.png)
