...
假设检验是一种有重要应用价值的统计推断形式。从理论上说，它是数理统计学的一个重要分支。假设检验方法的理论和系统发展，始于20世纪初，在历史上经历了以下一些重大事件：
...
“显著性检验”这个名词还可以从更实际的意义上去理解。在工农业生产中，假设加盐问题常在这种情况下出现：有两个处理（如两种工艺流程、两个种子品种、两种施肥方法等）$A$和$B$。$A$是原有的，已用了相当长的一个时期；$B$是新的，设想是一种改进。我们要通过试验来判断$B$是否优于$A$，于是引进假设$H$：$A$与$B$效果一样。
