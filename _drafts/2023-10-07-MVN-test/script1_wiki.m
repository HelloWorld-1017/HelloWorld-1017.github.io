% https://real-statistics.com/multivariate-statistics/multivariate-normal-distribution/multivariate-normality-testing/
clc,clear,close all
...
for ii = 1:n
    for jj = 1:n
        A = A+(1/6/n)*((meas(ii,:)-mu)*inv(Sigma_hat)*(meas(jj,:)-mu)')^3;
        count1 = count1+1;
        if ii == jj
            B = B+sqrt(n/(8*k*(k+2)))*((1/n)*((meas(ii,:)-mu)*inv(Sigma_hat)*(meas(jj,:)-mu)')^2-k*k-2);
            count2 = count2+1;
        end
    end
end
