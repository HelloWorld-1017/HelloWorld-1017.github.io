...
其中，$X^{\dagger}=(X^TX)^{-1}X^T$ 称为 $X$ 的伪逆。实际上，矩阵 $X^TX$ 也可能不可逆，这时我们需要直接求解$\eqref{LS}$ 所对应的线性方程组，详见博客：[超定方程的定义和求解](http://www.whatastarrynight.com/blog/Overdetermined-system/)。
