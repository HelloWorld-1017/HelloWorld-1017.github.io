---
layout: single
title: Constructing A Simple CNN for Solving MNIST Image Classification with PyTorch
...
`torch.nn.Conv2d`还有一个属性是`stride`，表示卷积核每次移动的步长：
...
但在一般情况下，我们在设计CNN的时候，是不用特别关注卷积核的size，**我们只需要保证前后相连的两个卷积层的通道数量匹配，CNN就可以正常计算下去**。
...
说明PyTorch不会对这种情况进行自动地处理。此时，我们需要使用`padding`参数向输入补充0。

设置`padding=1`仍然不符合要求：
...
我们在前面提到过，卷积层和池化层是不太在乎输入的大小，在一般的情况下都可以进行计算，不会报错；但是当卷积部分的最后一层输出展开成一个向量时，我们需要知道展开成了一个多长的向量，以设置全连接层（分类器）神经元的个数。
...
在整个CNN中，具有训练参数的层有三个，包括两个卷积层和一个全连接层，可训练的参数一共有：
