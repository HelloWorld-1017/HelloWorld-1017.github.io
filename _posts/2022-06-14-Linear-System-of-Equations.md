...
这个方程解的形式依赖于矩阵A所代表的变换是**将空间挤压到一条直线或点等低维空间**，还是**像初始状态一样的完整的二维空间**。根据行列式所代表的含义 [[3]](#ref)，上述两种情况分别对应：$A$ 的行列式为零和 $A$ 的行列式不为零。
...
仍以二维空间为例。我们之前提到过，高维空间投影到数轴上操作实际上是一种线性变换 [[4]](#ref)。假设线性变换$A$使得某一向量投影在一维空间的数轴$x_3$，变换后的向量记作$\boldsymbol{v}$。
