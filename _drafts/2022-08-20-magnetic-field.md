...
Biot-Savat 用于计算由电流（如导线中的电流）产生的在三维空间中 $\boldsymbol{r}$ 处的合成磁场 $\boldsymbol{\mathrm{B}}$ 。稳定电流(stationary current)是电荷的连续流动，它不随时间变换，并且电荷在任何电都不积累也不消耗。该定律是线积分(line integral)的一个物理实例，在电流流动的路径 $C$ 上进行计算。在 SI 单位制下，该方程可以写作：
$$
