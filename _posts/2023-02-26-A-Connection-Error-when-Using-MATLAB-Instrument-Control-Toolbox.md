...
**另外，为什么最终是锁定关闭MATLAB程序呢？**因为之前我在所导出的`tasklist.csv`中文件搜索`picoscope`关键词时，找到了一个窗口标题为`PicoScope ......`的进程，而该进程的映像名称就是`MATLAB.exe`，因此我将其关闭后，就解决了问题；现在再尝试时，`MATLAB.exe`所对应的窗口标题就是`MATLAB R2022a - acdemic use`，尽管如此，我将该进程关闭后依然有用。
