...
![image-20220826135350929](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/img/image-20220826135350929.png)
...
% Get mmf, phi, x
unit1 = simlog_ssc_solenoid_magnetic.Solenoid_with_Magnetic_Effects.Reluctance_Force_Actuator.mmf.series.unit;% [A]
unit2 = simlog_ssc_solenoid_magnetic.Solenoid_with_Magnetic_Effects.Reluctance_Force_Actuator.phi.series.unit;% [Wb]
unit3   = simlog_ssc_solenoid_magnetic.Solenoid_with_Magnetic_Effects.Reluctance_Force_Actuator.x.series.unit;% [mm]
mmf = simlog_ssc_solenoid_magnetic.Solenoid_with_Magnetic_Effects.Reluctance_Force_Actuator.mmf.series.values;     % [A]
phi = simlog_ssc_solenoid_magnetic.Solenoid_with_Magnetic_Effects.Reluctance_Force_Actuator.phi.series.values;     % [Wb]
x   = simlog_ssc_solenoid_magnetic.Solenoid_with_Magnetic_Effects.Reluctance_Force_Actuator.x.series.values*1e-3;  % [m]
% mmf(1:100)=[];
% phi(1:100) = [];
% x(1:100) = [];
...
在整个计算实现的步骤中，个人( •̀ ω •́ )✧**猜想**( •̀ ω •́ )✧可能是因为磁欧姆定律对于这部分不再适用，即代码中：
...
是不合理的。这个“元件”是动态的，就像电感元件一样，电感两端的电流和电压在🙋‍♂️**实数域上**🙋‍♂️不满足比例关系。再观察一下`Reluctance Force Actuator`模块和磁阻模块源文件的差异：





另一方面， $\dfrac{\mathrm{d}R}{\mathrm{d}x}=\dfrac{\mathrm{d}R}{\mathrm{d}t}\dfrac{\mathrm{d}t}{\mathrm{d}x}=\dfrac1v\dfrac{\mathrm{d}R}{\mathrm{d}t}$是成立的，通过类似的方式得到了验证：
...
![image-20220826110913989](https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/img/image-20220826110913989.png)

如果增加`Contact stiffness`参数的值，可以使所构建的刚体模型更加理想：

<figure class="half">
    <img src="https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/img/image-20220826111505616.png">
    <img src="https://blogimages-1309804558.cos.ap-nanjing.myqcloud.com/img/image-20220826111420012.png">
